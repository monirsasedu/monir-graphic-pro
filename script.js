...
const texts = ["Developer.", "Professional Coder.", "Graphic Designer.", "UI/UX Specialist.", "ads Specialist"]; // Add more titles here
...
function typeAnimation() {
  ...
  if (!isDeleting && charIndex === currentText.length) {
    setTimeout(() => (isDeleting = true), 1000); // Pause after completing the word
  }
    ...
    (entries, observer) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          startCounting(); // Trigger the counting when the container is visible
    ...
    counters.forEach(counter => {
      ...
      function updateCounter() {
        ...
        if (value < target) {
          ...
          setTimeout(updateCounter, 10); // Adjust interval here
        } else {
